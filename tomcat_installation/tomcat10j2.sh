...
#                Backs up existing installation before reinstalling.
...
# Backup Dir   : /usr/share/tomcat10_backup_<timestamp>
...
BACKUP_DIR="/usr/share/${TOMCAT_DIR}_backup_$(date +%Y%m%d_%H%M%S)"
...
# Preserves server.xml, setenv.sh, and other configuration files
###############################################################################
backup_existing_tomcat() {
    log_msg "INFO" "Creating backup of existing Tomcat installation..."
    
    # Create backup directory
    mkdir -p "${BACKUP_DIR}"
    
    # Copy entire Tomcat installation to backup
    cp -a "${TOMCAT_HOME}/." "${BACKUP_DIR}/" || \
        error_exit "Failed to backup existing Tomcat installation"
    
    # Set permissions on backup
    chmod -R 755 "${BACKUP_DIR}"
    
    log_msg "INFO" "Backup created successfully at: ${BACKUP_DIR}"
    
    # List important configuration files in backup
    echo ""
    echo "============================================================================="
    echo "BACKUP CREATED - Important Configuration Files:"
    echo "============================================================================="
    echo "Backup Location  : ${BACKUP_DIR}"
    echo ""
    echo "Key files preserved for reference:"
    
    if [[ -f "${BACKUP_DIR}/conf/server.xml" ]]; then
        echo "  - ${BACKUP_DIR}/conf/server.xml"
    fi
    
    if [[ -f "${BACKUP_DIR}/bin/setenv.sh" ]]; then
        echo "  - ${BACKUP_DIR}/bin/setenv.sh"
    fi
    
    if [[ -f "${BACKUP_DIR}/conf/context.xml" ]]; then
        echo "  - ${BACKUP_DIR}/conf/context.xml"
    fi
    
    if [[ -f "${BACKUP_DIR}/conf/tomcat-users.xml" ]]; then
        echo "  - ${BACKUP_DIR}/conf/tomcat-users.xml"
    fi
    
    if [[ -f "${BACKUP_DIR}/conf/web.xml" ]]; then
        echo "  - ${BACKUP_DIR}/conf/web.xml"
    fi
    
    if [[ -f "${BACKUP_DIR}/conf/logging.properties" ]]; then
        echo "  - ${BACKUP_DIR}/conf/logging.properties"
    fi
    
    # Check for any webapps
    if [[ -d "${BACKUP_DIR}/webapps" ]]; then
        local webapp_count
        webapp_count=$(find "${BACKUP_DIR}/webapps" -maxdepth 1 -type d | wc -l)
        webapp_count=$((webapp_count - 1))  # Subtract 1 for the webapps dir itself
        if [[ $webapp_count -gt 0 ]]; then
            echo "  - ${BACKUP_DIR}/webapps/ ($webapp_count application(s))"
        fi
    fi
    
    echo ""
    echo "To restore configuration after installation:"
    echo "  cp ${BACKUP_DIR}/conf/server.xml ${TOMCAT_HOME}/conf/"
    echo "  cp ${BACKUP_DIR}/bin/setenv.sh ${TOMCAT_HOME}/bin/"
    echo "============================================================================="
    echo ""
    
    log_msg "INFO" "Backup summary displayed"
}

###############################################################################
# Check for existing Tomcat installation
###############################################################################
check_existing_tomcat() {
    log_msg "INFO" "Checking for existing Tomcat installation..."
    
    if [[ -d "${TOMCAT_HOME}" ]]; then
        log_msg "WARN" "Existing Tomcat installation found at: ${TOMCAT_HOME}"
        
        # Check if service is running
        if systemctl is-active "${TOMCAT_DIR}" &>/dev/null; then
            log_msg "WARN" "Tomcat service is currently running"
        fi
        
        # Check for existing version
        if [[ -f "${TOMCAT_HOME}/lib/catalina.jar" ]]; then
            local existing_version
            existing_version=$(unzip -p "${TOMCAT_HOME}/lib/catalina.jar" META-INF/MANIFEST.MF 2>/dev/null | grep "Implementation-Version" | cut -d' ' -f2 | tr -d '\r' || echo "unknown")
            log_msg "INFO" "Existing Tomcat version: ${existing_version}"
        fi
        
        echo ""
        read -p "Do you want to backup existing installation and reinstall? (y/N): " REINSTALL || REINSTALL="n"
        ...
        if systemctl is-active "${TOMCAT_DIR}" &>/dev/null; then
            log_msg "INFO" "Stopping existing Tomcat service..."
            systemctl stop "${TOMCAT_DIR}" || true
        fi
        ...
        log_msg "INFO" "No existing Tomcat installation found"
    fi
}
...
extract_tomcat() {
    ...
    # Remove existing installation if present (already backed up)
    if [[ -d "${TOMCAT_HOME}" ]]; then
        log_msg "INFO" "Removing existing Tomcat installation (backup already created)..."
        rm -rf "${TOMCAT_HOME}"
    fi
...
print_summary() {
    ...
    if [[ -d "${BACKUP_DIR}" ]]; then
        ...
        echo "  Backup Location: ${BACKUP_DIR}"
        echo "  Restore configs: cp ${BACKUP_DIR}/conf/* ${TOMCAT_HOME}/conf/"
        echo "  Restore setenv : cp ${BACKUP_DIR}/bin/setenv.sh ${TOMCAT_HOME}/bin/"
        echo ""
    fi
