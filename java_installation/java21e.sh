...
# Supported OS : Amazon Linux 2/2023, RHEL 7/8/9, CentOS 7/8, Fedora, Oracle Linux
###############################################################################
...
# Required for: RHEL, CentOS, Fedora, Oracle Linux
...
configure_repository() {
    ...
    if [[ "$ID" == "rhel" || "$ID" == "amzn" || "$ID" == "ol" || "$ID" == "fedora" || "$ID" == "centos" ]]; then
...
# FIXED VERSION - Ensures Java 21 takes precedence
###############################################################################
configure_system_java_home() {
    log_msg "INFO" "Configuring system-wide JAVA_HOME..."
    
    # Remove any old Java environment files to avoid conflicts
    sudo rm -f /etc/profile.d/java.sh
    sudo rm -f /etc/profile.d/java8.sh
    
    sudo tee /etc/profile.d/java21.sh > /dev/null << 'EOF'
# Amazon Corretto Java 21 Environment
# Auto-generated by install_java21_corretto.sh

# Remove any existing Java paths from PATH to avoid conflicts
PATH=$(echo "$PATH" | sed -e 's|:/usr/lib/jvm/[^:]*bin||g' -e 's|/usr/lib/jvm/[^:]*bin:||g')

# Set Java 21 as default
export JAVA_HOME=/usr/lib/jvm/java-21-amazon-corretto.x86_64
export PATH=$JAVA_HOME/bin:$PATH

# Ensure Java 21 binaries take precedence
...
EOF
    ...
    log_msg "INFO" "Created /etc/profile.d/java21.sh with PATH cleanup"
}

###############################################################################
# Update user's .bashrc - MODIFIED to NOT set JAVA_HOME
# Since we're using profile.d, we only need to comment out old Java 8 config
###############################################################################
update_user_bashrc() {
    ...
    for user_home in "${homes_to_update[@]}"; do
        ...
        # Comment out any existing JAVA_HOME settings (Java 8 or Java 21)
        if grep -q "JAVA_HOME.*java.*corretto" "$bashrc_file"; then
            log_msg "INFO" "Found Java configuration in .bashrc, commenting out..."
            
            # Comment out all JAVA_HOME lines related to Java
            sed -i 's|^export JAVA_HOME=.*java.*corretto.*$|# [Managed by /etc/profile.d/java21.sh] &|' "$bashrc_file"
            sed -i 's|^JAVA_HOME=.*java.*corretto.*$|# [Managed by /etc/profile.d/java21.sh] &|' "$bashrc_file"
            
            # Also comment out PATH modifications for Java
            sed -i 's|^export PATH=\$JAVA_HOME/bin:\$PATH.*$|# [Managed by /etc/profile.d/java21.sh] &|' "$bashrc_file"
            sed -i 's|^PATH=\$JAVA_HOME/bin:\$PATH.*$|# [Managed by /etc/profile.d/java21.sh] &|' "$bashrc_file"
            
            log_msg "INFO" "Java configuration commented out (now managed by /etc/profile.d/java21.sh)"
        fi
        
        # Remove any Java 21 blocks previously added by this script
        if grep -q "# \[Java Corretto 21\]" "$bashrc_file"; then
            log_msg "INFO" "Removing old Java 21 block..."
            sed -i '/# \[Java Corretto 21\]/,/^export PATH=\$JAVA_HOME\/bin:\$PATH$/d' "$bashrc_file"
        fi
        
        # Add a note that Java is managed by profile.d
        if ! grep -q "Java is managed by /etc/profile.d/java21.sh" "$bashrc_file"; then
            cat >> "$bashrc_file" << 'EOF'

# Java Environment
# Java is managed by /etc/profile.d/java21.sh for system-wide consistency
# To override, export JAVA_HOME after this line
EOF
        fi
        
        log_msg "INFO" "Updated $bashrc_file - Java now managed by /etc/profile.d/"
    done
}
...
print_switching_instructions() {
    if [[ "$JAVA8_INSTALLED" == "true" ]]; then
        ...
        echo ""
        echo "Option 1: System-wide (edit /etc/profile.d/java21.sh)"
        echo "  Edit the JAVA_HOME path in /etc/profile.d/java21.sh"
        echo "  Change to: export JAVA_HOME=${JAVA8_HOME}"
        echo "  Then log out and log back in"
        echo ""
        echo "Option 2: Per-session override"
        ...
        echo ""
        echo "Option 3: Use alternatives (requires root)"
        echo "  sudo alternatives --config java"
        echo "  sudo alternatives --config javac"
        echo ""
        echo "NOTE: To revert .bashrc to previous state, restore from backup:"
...
print_summary() {
    ...
    echo ""
    echo "CONFIGURATION:"
    echo "  System-wide    : /etc/profile.d/java21.sh (PRIMARY - manages JAVA_HOME)"
    echo "  User .bashrc   : Java settings commented out (defers to profile.d)"
    ...
    echo "  Option 2: Run: source /etc/profile.d/java21.sh"
    echo ""
